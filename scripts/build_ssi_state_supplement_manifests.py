...
# ---------------------------------------------------------------- batch 5: re-probe of the blocked rows (2026-09-13)
# One plain GET of each blocked row's recorded URL with the extractor client from a US network
# (docs/ingest-runs/2026-09-13-blocked-publishers-reprobe.md); appended to the row notes, nothing else changes.
REPROBE_STAMP = "2026-09-13T19:57Z"
REPROBE_NOTES_5 = {
    "NY": (f"Re-probed {REPROBE_STAMP} from a US network with the plain extractor client, same failure: connection reset by peer after "
           "0.1 s, no response. Durable block: the same failure from two networks (the 2026-09-10 non-US and US exits) on two dates "
           "(2026-09-10 and 2026-09-13); the dashboard should treat the cell as not available rather than pending."),
    "WY": (f"Re-probed {REPROBE_STAMP} from a US network with the plain extractor client: ecom.wyo.gov answers HTTP 200 (218,582 bytes, "
           "0.3 s) with the same Google Sites navigation and script-rendered Drive embed; M1804 is still not addressable from the "
           "publisher's page. Not a network block: the document is not fetchable on two dates (2026-09-11 and 2026-09-13); treat the "
           "cell as not available rather than pending."),
    **dict.fromkeys(("PR", "GU", "VI", "AS"),
                    f"Re-probed {REPROBE_STAMP} from a US network with the plain extractor client: POMS SI 00501 subchapter list HTTP 200 "
                    "(48,992 bytes, 0.3 s) and 42 U.S.C. 1382c on uscode.house.gov HTTP 200 (228,944 bytes, 4.5 s); the statutory "
                    "exclusion is unchanged. Not applicable on two dates; nothing to take."),
}


def update_queue_batch5(only: set[str] | None = None) -> dict:
    """Append the 2026-09-13 re-probe finding to each blocked row's notes (no builders: nothing is fetched)."""
    queue = yaml.safe_load(QUEUE.read_text())
    rows: dict[str, dict] = {}
    for row in queue["states"]:
        rows.setdefault(row["jurisdiction"], row)
    for code, note in REPROBE_NOTES_5.items():
        if only and code not in only:
            continue
        row = rows[f"us-{code.lower()}"]
        if row["queue_status"] != "blocked_primary_source":
            raise SystemExit(f"{code}: not a blocked row ({row['queue_status']}); nothing appended")
        if note not in row["notes"]:
            row["notes"] = row["notes"].rstrip() + " " + note
    queue["status_counts"] = {}
    for row in queue["states"]:
        queue["status_counts"][row["queue_status"]] = queue["status_counts"].get(row["queue_status"], 0) + 1
    batch_note = (
        "2026-09-13 SSI re-probe of the blocked rows: scripts/build_ssi_state_supplement_manifests.py --batch 5; NY still reset by "
        "otda.ny.gov (durable block), WY still a script-rendered Drive embed, PR/GU/VI/AS still outside title XVI; nothing fetched "
        "beyond one plain GET each; docs/ingest-runs/2026-09-13-blocked-publishers-reprobe.md."
    )
    notes = queue.setdefault("policy", {}).setdefault("notes", [])
...
def main() -> int:
    ...
    if args.batch in (4, 5):
        ...
        print("queue status_counts:", (update_queue_batch4 if args.batch == 4 else update_queue_batch5)(only))
        return 0
