...
def _release_payload(repo: Path, release_ref: str, release_path: Path) -> dict[str, Any]:
    if release_ref == "WORKTREE":
        payload = json.loads((repo / release_path).read_text())
    else:
...
def main() -> int:
    ...
    output = {
        ...
        "absent_manifest_count": sum(not row["manifest_exists"] for row in report),
        "unsigned_stub_count": sum(
