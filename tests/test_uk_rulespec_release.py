...
UK_RELEASE_PATH = REPO_ROOT / "manifests" / "releases" / "uk-rulespec-current.json"
...
def test_uk_rulespec_selector_is_the_exact_current_uk_release():
    release = ReleaseManifest.load(UK_RELEASE_PATH)
    current = ReleaseManifest.load(CURRENT_RELEASE_PATH)
    current_uk_keys = tuple(
        scope.key
        for scope in current.scopes
        if scope.jurisdiction == "uk" or scope.jurisdiction.startswith("uk-")
    )

    assert release.scope_keys == EXPECTED_SCOPE_KEYS
    assert current_uk_keys == EXPECTED_SCOPE_KEYS
...
def test_rulespec_uk_migration_preserves_every_body_url_and_source_snapshot():
    ...
    assert migration["active_release"] == "manifests/releases/uk-rulespec-current.json"
