...
- Push branches to this repository, never to a fork: a fork-head pull request run does not receive this repository's Actions variable `AXIOM_CORPUS_INGEST_PUBLIC_KEY`, so the authentication-gate tests in `tests/test_state_snap_manual_queue.py` fail by design. See CONTRIBUTING.md.
...
This project is indexed by GitNexus as **mirror-wf** (18335 symbols, 51870 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
3. `READ gitnexus://repo/mirror-wf/process/{processName}` — trace the full execution flow step by step
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/mirror-wf/context` | Codebase overview, check index freshness |
| `gitnexus://repo/mirror-wf/clusters` | All functional areas |
| `gitnexus://repo/mirror-wf/processes` | All execution flows |
| `gitnexus://repo/mirror-wf/process/{name}` | Step-by-step execution trace |
