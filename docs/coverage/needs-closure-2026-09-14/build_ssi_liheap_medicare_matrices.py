#!/usr/bin/env python3
"""Needs-driven closure check for SSI, LIHEAP and Medicare (2026-09-14 re-run).

Single source of truth for the three law-derived needs schemas and the
jurisdiction x element matrices under docs/coverage/needs-closure-2026-09-14/.
The element lists are identical to the 2026-09-11 pass so the numbers compare;
the scope pointers are resolved against --selector (the sixth cut) and the
cells whose carrying family that cut newly holds read the corpus instead of the
2026-09-11 EXTRACTABLE/REVIEW literals.

Reads corpus artifacts read-only from --base (default: the main checkout's
data/corpus). Writes <program>-schema.yaml and <program>-matrix.csv to --out
(default: next to this script) and prints the roll-ups used in <program>.md.
...
# Selected federal scopes (manifests/releases/us-rulespec-2026-09-14-wave4-r2-union.json, the
# sixth cut). Every version here is checked against --selector at start-up.
SEL = {
    "ssi_statute": "2026-06-20-ssi-title-xvi-title-42-r2026-07-15-self-contained-r2026-07-17-dedup",
    "ssi_statute2": "2026-09-13-ssi-statute-1381a-1383f-title-42",
    "cfr416": "2026-09-11-title-20-part-416",
    "poms": "2026-09-10-ssi-poms-si",
    "poms2": "2026-09-13-ssi-poms-si-remainder",
    "cola": "2026-07-05-ssa-cola-2026",
    "autodet": "2026-05-17-ssa-automatic-determinations-2026",
    "dc_ossp": "2026-07-03-dc-ossp-ssa-poms",
    "liheap_statute": "2026-09-13-liheap-statute-chapter-94-title-42",
    "cfr96": "2026-09-13-title-45-part-96",
    "liheap_ann": "2026-09-13-liheap-annual-figures-2026",
    "m426": "2026-06-23-medicare-426-title-42-r2026-07-15-self-contained-r2026-07-17-dedup",
    "med_statute": "2026-09-13-medicare-statute-eligibility-premiums-title-42",
    ...
    "cfr42": "2026-09-13-title-42-part-",  # prefix: parts 406, 407, 408, 423
    ...
    "iom0116": "2026-09-13-medicare-cms-iom-100-16",
    "iom0118": "2026-09-13-medicare-cms-iom-100-18",
    "poms_hi": "2026-09-13-medicare-poms-hi",
    "med_ann": "2026-09-13-medicare-annual-notices-2026",
...
SELECTED: set[tuple[str, str, str]] = set()  # (jurisdiction, document_class, version) from --selector
SEL_BY_JC: dict[tuple[str, str], list[str]] = {}


def load_selector(path: Path):
    for s in json.load(path.open())["scopes"]:
        k = (s["jurisdiction"], s["document_class"], s["version"])
        SELECTED.add(k)
        SEL_BY_JC.setdefault((s["jurisdiction"], s["document_class"]), []).append(s["version"])


def resolve_scope(jur: str, cls: str, ver: str, cp: str = "") -> str:
    """The selected version a 2026-09-11 scope pointer resolves to: the version itself when it
    is selected; else a selected consolidated version whose name starts with it (a trailing
    '-consolidated' is dropped before the prefix test, e.g. us-oh 5101-4-...-5122-36-consolidated
    -> ...-5122-36-r2026-09-14-5703-7-consolidated); else, when a citation path is given, the
    selected scope of the same jurisdiction and class that holds that path (e.g. the CT SSP UPM
    rows now live in us-ct/policy/2026-09-14-tanf-manual-whole). Falls back to the pointer
    unchanged so verify_matrices.py reports it."""
    if (jur, cls, ver) in SELECTED:
        return ver
    stem = ver[: -len("-consolidated")] if ver.endswith("-consolidated") else ver
    for v in SEL_BY_JC.get((jur, cls), []):
        if v.startswith(stem) and (not cp or any(d["citation_path"] == cp for d in rows(BASE, f"{jur}/{cls}/{v}"))):
            return v
    if cp:
        for v in SEL_BY_JC.get((jur, cls), []):
            if any(d["citation_path"] == cp for d in rows(BASE, f"{jur}/{cls}/{v}")):
                return v
    return ver


def check_sel(base: Path) -> list[str]:
    """The promise above SEL: every version there is selected (a trailing '-' is a prefix that must match at
    least one selected us/regulation version) and has a provisions file under --base. Problems are printed
    as warnings on stderr and returned; the build still runs so the matrices show what resolves."""
    problems = []
    for key, ver in SEL.items():
        if ver.endswith("-"):
            hits = [v for v in SEL_BY_JC.get(("us", "regulation"), []) if v.startswith(ver)]
            if not hits:
                problems.append(f"SEL[{key}] prefix {ver}: no selected us/regulation version starts with it")
            for v in hits:
                if not (base / "provisions" / "us" / "regulation" / f"{v}.jsonl").exists():
                    problems.append(f"SEL[{key}] {v}: selected but no provisions file under {base}")
            continue
        classes = [c for (j, c, v) in SELECTED if j == "us" and v == ver]
        if not classes:
            problems.append(f"SEL[{key}] {ver}: not in --selector")
        elif not any((base / "provisions" / "us" / c / f"{ver}.jsonl").exists() for c in classes):
            problems.append(f"SEL[{key}] {ver}: selected but no provisions file under {base}")
    for msg in problems:
        print(f"WARNING: {msg}", file=sys.stderr)
    return problems


def held_path(base: Path, rel: str, cp: str):
    """cp when the scope holds it with a non-empty body, else its first descendant row with a body, else None."""
    for d in rows(base, rel):
        if d["citation_path"] == cp and (d.get("body") or "").strip():
            return cp
    for d in rows(base, rel):
        if d["citation_path"].startswith(cp + "/") and (d.get("body") or "").strip():
            return d["citation_path"]
    return None
...
def norm_cp(cp: str) -> str:
    """The 2026-09-13 Medicare statute scope writes hyphenated section numbers with an en dash
    (us/statute/42/426–1); compare on the ASCII hyphen."""
    return cp.replace("–", "-")


def best_dollar_row(base: Path, rel: str, minimum: int = 3):
    """The row of a scope with the most dollar figures (a benefit matrix or standards table);
    None when no row reaches `minimum`."""
    best, n_best = None, 0
    for d in rows(base, rel):
        n = len(re.findall(r"\$\s?\d", d.get("body") or ""))
        if n > n_best:
            best, n_best = d, n
    return (best, n_best) if n_best >= minimum else (None, 0)


def first_body_row(base: Path, rel: str, min_chars: int = 200):
    for d in rows(base, rel):
        if len((d.get("body") or "").strip()) >= min_chars:
            return d
    return next((d for d in rows(base, rel) if (d.get("body") or "").strip()), None)
...
class Matrix:
    ...
    def add(self, jur, element, level, family, status, scope="", cp="", note="", pe="no"):
        ...
        if status == "PRESENT":
            ...
            scope = resolve_scope(cp.split("/")[0], cp.split("/")[1], scope, cp)
...
def build_ssi(base: Path):
    ...
    scope_st2 = SEL["ssi_statute2"]  # 2026-09-13 remainder: 1381a and 1383-1383f
    held2 = {d["citation_path"] for d in rows(base, f"us/statute/{scope_st2}")}
    # statute
    for sec, title, pe in SSI_STATUTE:
        ...
        if cp in held:
            ...
            n = sum(1 for d in rows(base, f"us/statute/{scope_st2}") if d["citation_path"].startswith(cp + "/") or d["citation_path"] == cp)
            m.federal(eid, "federal", "statute", "PRESENT", scope_st2, held_path(base, f"us/statute/{scope_st2}", cp), f"{n} rows in the 2026-09-13 SSI statute remainder scope (1381a, 1383-1383f); subsection bodies opened", pe)
        else:
    ...
    app = next((r for r in rows(base, f"us/regulation/{SEL['cfr416']}") if (r.get("kind") == "appendix" or "appendix" in r["citation_path"].lower()) and (r.get("body") or "").strip()), None)
    if app:
        m.federal(eid, "federal", "regulation", "PRESENT", SEL["cfr416"], app["citation_path"], "appendix row held in the selected part 416 scope; body opened")
    else:
    ...
    for sub, title, n, taken in POMS_SI:
        ...
        if taken:
            ...
            rel2 = f"us/manual/{SEL['poms2']}"
            blk = rf"^us/manual/ssa/poms/si/{sub}\.\d{{3}}/block-\d+$"
            d = find(base, rel2, blk, r".{200,}") or find(base, rel2, blk, r"\S")
            if d:
                nsec = sum(1 for r in rows(base, rel2) if r.get("kind") == "document" and re.match(rf"^us/manual/ssa/poms/si/{sub}\.\d{{3}}$", r["citation_path"]))
                nblk = sum(1 for r in rows(base, rel2) if re.match(blk, r["citation_path"]) and (r.get("body") or "").strip())
                m.federal(eid, "federal", "manual", "PRESENT", SEL["poms2"], d["citation_path"], f"{nsec} sections ({nblk} blocks with text) in the 2026-09-13 POMS SI remainder scope (index lists {n}); block bodies opened", pe)
            else:
                ch = sub[:3]
...
def build_liheap(base: Path):
    schema = []
    m = Matrix()
    rel_ls = f"us/statute/{SEL['liheap_statute']}"
    held_ls = {d["citation_path"] for d in rows(base, rel_ls)}
    for sec, title in LIHEAP_STATUTE:
        ...
        cp = f"us/statute/42/{sec}"
        if cp in held_ls:
            n = sum(1 for d in rows(base, rel_ls) if d["citation_path"] == cp or d["citation_path"].startswith(cp + "/"))
            m.federal(eid, "federal", "statute", "PRESENT", SEL["liheap_statute"], held_path(base, rel_ls, cp), f"{n} rows in the 2026-09-13 Title 42 ch. 94 scope; subsection bodies opened", pe)
        else:
    ...
    for sec, title in LIHEAP_CFR:
        ...
        rel96 = f"us/regulation/{SEL['cfr96']}"
        cp = f"us/regulation/45/96/{sec.split('.')[1]}"
        subh = sorted(r["citation_path"].rsplit("/", 1)[1] for r in rows(base, rel96) if r.get("kind") == "section" and (r.get("metadata") or {}).get("subpart") == "H" and (r.get("body") or "").strip())
        if held_path(base, rel96, cp):
            m.federal(eid, "federal", "regulation", "PRESENT", SEL["cfr96"], held_path(base, rel96, cp), f"section row in the 2026-09-13 eCFR part 96 scope (subpart H holds {len(subh)} sections: 96.{', 96.'.join(subh)}); body opened", pe)
        else:
            m.federal(eid, "federal", "regulation", "EXTRACTABLE", "", "", f"no {cp} row in the held 2026-09-13 part 96 scope (its subpart H holds 96.{', 96.'.join(subh)}); eCFR Versioner API (same publisher as the 2026-09-11 part 98/436/416 scopes; adapter now sends Accept-Encoding)", pe)
    ann = [
    ...
    rel_la = f"us/guidance/{SEL['liheap_ann']}"
    ann_lookup = {  # element -> (citation_path regex, body regex, PRESENT note) in the 2026-09-13 LIHEAP annual-figures scope
        "LIHEAP-F-ANN-POVERTY": (r"^us/guidance/hhs/aspe/poverty-guidelines/2026/federal-register/block-1$", r"poverty guidelines",
                                 "91 FR 1797 (January 15, 2026) annual update of the HHS poverty guidelines; the ASPE page and LIHEAP IM 2026-01 attachment 2 (100/110/150 percent tables) are in the same scope; bodies opened"),
        "LIHEAP-F-ANN-SMI": (r"^us/guidance/acf/ocs/liheap-im/2026-01/attachment-4-smi-table/page-1$", r"State Median Income",
                             "LIHEAP IM 2026-01 attachment 4: SMI by household size (optional FY26, mandatory FY27); memorandum pages 1-4 in the same scope; bodies opened"),
        "LIHEAP-F-GUID-MODELPLAN": (r"^us/guidance/acf/ocs/liheap-im/", r"model plan", "LIHEAP IM 2026-01 held; body opened"),
    }
    for eid, name, fam, pe, note in ann:
        schema.append(elem(eid, name, "federal", fam, "HHS", pe, ["42 U.S.C. 8624(b)(2)", "45 CFR 96.85"], ""))
        cprx, brx, pnote = ann_lookup[eid]
        d = find(base, rel_la, cprx, brx)
        if d:
            m.federal(eid, "federal", fam.split(" (")[0], "PRESENT", SEL["liheap_ann"], d["citation_path"], pnote, pe)
        elif eid == "LIHEAP-F-GUID-MODELPLAN":
            m.federal(eid, "federal", fam.split(" (")[0], "EXTRACTABLE", "", "", "Model Plan form and instructions not in the corpus (only LIHEAP IM 2026-01 is held, in the annual-figures scope); ACF OCS LIHEAP policy-guidance index", pe)
        else:
...
def build_medicare(base: Path):
    schema = []
    m = Matrix()
    rel_ms = f"us/statute/{SEL['med_statute']}"
    held_ms: dict[str, str] = {}  # ASCII-hyphen path -> path as written (en dash) in the 2026-09-13 scope
    for d in rows(base, rel_ms):
        held_ms.setdefault(norm_cp(d["citation_path"]), d["citation_path"])
    for sec, title, pe, key in MEDICARE_STATUTE:
        ...
        elif f"us/statute/42/{sec}" in held_ms:
            cp = f"us/statute/42/{sec}"
            real = held_ms[cp]
            n = sum(1 for d in rows(base, rel_ms) if norm_cp(d["citation_path"]) == cp or norm_cp(d["citation_path"]).startswith(cp + "/"))
            m.federal(eid, "federal", "statute", "PRESENT", SEL["med_statute"], held_path(base, rel_ms, real), f"{n} rows in the 2026-09-13 Medicare eligibility/premiums statute scope" + (f" (section path written with an en dash: {real})" if real != cp else "") + "; subsection bodies opened", pe)
        else:
    ...
    for code, title, pe in MEDICARE_CFR:
        ...
        part, _, letter = code.partition("-")
        scope42 = f"{SEL['cfr42']}{part}"
        rel42 = f"us/regulation/{scope42}"
        secs = [r for r in rows(base, rel42) if r.get("kind") == "section" and (not letter or (r.get("metadata") or {}).get("subpart") == letter) and (r.get("body") or "").strip()]
        if secs:
            subs = sorted({(r.get("metadata") or {}).get("subpart") or "" for r in rows(base, rel42) if r.get("kind") == "section"}, key=lambda x: (len(x), x))
            head = f"subpart {letter}: {len(secs)} sections" if letter else f"part {part}: {len(secs)} sections (subparts {subs[0]}-{subs[-1]})"
            extra = "; 423.286 (premium rules, IRMAA) sits in eCFR subpart F of the same scope" if code == "423-D" else ""
            m.federal(eid, "federal", "regulation", "PRESENT", scope42, secs[0]["citation_path"], f"{head} with non-empty bodies in the 2026-09-13 eCFR scope (cited: first section; subpart container rows have no body by design){extra}", pe)
        else:
    ...
    for code, title, note in IOM_NOT_TAKEN:
        ...
        key = {"100-16": "iom0116", "100-18": "iom0118"}.get(code)
        d = find(base, f"us/manual/{SEL[key]}", rf"^us/manual/cms/iom/{code}/chapter-[^/]+/[^/]+$", r".{300,}") if key else None
        if d:
            allr = rows(base, f"us/manual/{SEL[key]}")
            nch = sum(1 for r in allr if r.get("kind") == "document")
            nsec = sum(1 for r in allr if r.get("kind") == "section" and (r.get("body") or "").strip())
            m.federal(eid, "federal", "manual", "PRESENT", SEL[key], d["citation_path"], f"{nch} chapters, {nsec} section rows with text in the 2026-09-13 scope; section bodies opened", "no")
        else:
    ...
    for code, title in POMS_HI:
        ...
        sub = code.split("-")[1]
        subrx = r"030(?!00)\d\d" if code == "HI-03001" else sub  # HI 03001-03050 is one element (03000 is the chapter TOC)
        rel_hi = f"us/manual/{SEL['poms_hi']}"
        blk = rf"^us/manual/ssa/poms/hi/{subrx}\.\d{{3}}/block-\d+$"
        d = find(base, rel_hi, blk, r".{200,}") or find(base, rel_hi, blk, r"\S")
        if d:
            nsec = sum(1 for r in rows(base, rel_hi) if r.get("kind") == "document" and re.match(rf"^us/manual/ssa/poms/hi/{subrx}\.\d{{3}}$", r["citation_path"]))
            m.federal(eid, "federal", "manual", "PRESENT", SEL["poms_hi"], d["citation_path"], f"{nsec} sections in the 2026-09-13 POMS HI scope ({'HI 03001-03050' if code == 'HI-03001' else 'HI ' + sub}); block bodies opened", pe)
        else:
    ...
    rel_ma = f"us/guidance/{SEL['med_ann']}"
    rel_la = f"us/guidance/{SEL['liheap_ann']}"
    ann_lookup = {  # element -> (scope rel, citation_path regex, body regex, PRESENT note)
        "MED-F-ANN-AB": (rel_ma, r"^us/guidance/cms/part-b-premium-deductible/2026/federal-register/block-1$", r"income-related monthly adjustment",
                         "90 FR 52063 (Nov 19, 2025) CY 2026 Part B premium/deductible/IRMAA notice; the Part A premium (90 FR 52060) and Part A deductible/coinsurance (90 FR 52075) notices and the CMS fact sheet are in the same scope; bodies opened"),
        "MED-F-ANN-D": (rel_ma, r"^us/guidance/cms/part-d-national-average-bid-amount/2026/page-1$", r"base beneficiary premium",
                        "CY 2026 Part D national average bid amount / base beneficiary premium release; the Part D bid-information fact sheet and the CY 2026 MA/Part D rate announcement are in the same scope; bodies opened"),
        "MED-F-ANN-LIS": (rel_ma, r"^us/guidance/cms/part-d-lis-resource-limits/2026/page-1$", r"resource limit",
                          "CY 2026 LIS resource and cost-sharing limits memo; body opened (MSP resource standards are not restated in it)"),
        "MED-F-ANN-POVERTY": (rel_la, r"^us/guidance/hhs/aspe/poverty-guidelines/2026/federal-register/block-1$", r"poverty guidelines",
                              "91 FR 1797 (January 15, 2026) annual update of the HHS poverty guidelines, held in the 2026-09-13 LIHEAP annual-figures scope; body opened"),
    }
    for eid, name, pe, st, sc, cp, note in ann:
        ...
        rel, cprx, brx, pnote = ann_lookup[eid]
        d = find(base, rel, cprx, brx)
        if d:
            st, sc, cp, note = "PRESENT", rel.split("/")[-1], d["citation_path"], pnote
...
def main():
    ...
    ap.add_argument("--selector", default="/Users/pavelmakarchuk/axiom-corpus-worktrees/closure-r6/manifests/releases/us-rulespec-2026-09-14-wave4-r2-union.json",
                    help="named-release selector; every SEL version and every PRESENT scope is resolved against it")
    a = ap.parse_args()
    base = Path(a.base)
    BASE = base
    load_selector(Path(a.selector))
    problems = check_sel(base)
    print(f"selector {Path(a.selector).name}: {len(SELECTED)} scopes; SEL versions checked: {len(SEL)}, problems: {len(problems)}")
    builds = [
