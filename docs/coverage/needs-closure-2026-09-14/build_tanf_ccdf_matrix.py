#!/usr/bin/env python3
"""Needs-driven closure check for TANF and CCDF (2026-09-11 schema, re-run on a selector).
...
over the federal level plus the 50 states and DC, and a ``summary-tanf-ccdf.json``
with the roll-ups the ``<program>.md`` reports quote.

Read-only with respect to ``data/corpus``. Inputs:

* a named-release selector (``--selector``; default the 2026-09-14 wave-4 r2 union),
  the set of selected scopes; every scope table below is resolved against it
  (exact version, else the selected version of the same jurisdiction and class
  that starts with the table's version, else the selected scope that holds at
  least 90 percent of the table scope's body-bearing citation paths, i.e. a
  consolidation or supersede; anything unresolved is warned on stderr);
...
* provision bodies under ``data/corpus/provisions`` for the selected scopes
  (and, for TANF states whose July scopes are still unselected, those on-disk scopes);
...
The element lists (ids, names, levels, families, facts, patterns, pe_modeled) are
those of the 2026-09-11 check and must stay identical so the numbers compare;
only the corpus-status strings, the date and the method text are computed per run.
...
Usage: python3 build_tanf_ccdf_matrix.py --base /path/to/data/corpus --repo <worktree> \
    --selector manifests/releases/<name>.json --out <this dir>
"""
...
STRUCTURE = "sources/us/regulation/2026-09-11-title-45-part-98/ecfr/title-45.structure.json"
PART98 = ("us", "regulation", "2026-09-11-title-45-part-98")  # selected in the 2026-09-14 r2 union
SUMMARY_NAME = "summary-tanf-ccdf.json"
# Version suffixes of the scope families resolved per jurisdiction against the selector.
PLAN_SUFFIX = "ccdf-plan-fy2025-2027"
APPENDIX_SUFFIX = "ccdf-state-plan-appendices"
AMENDMENT_SUFFIX = "ccdf-state-plan-amendments"
RATE_SUFFIX = "ccdf-rate-schedules"
WHOLE_MANUAL_SUFFIX = "tanf-manual-whole"
...
# Which selected scopes per jurisdiction carry TANF text (searched for PRESENT). Versions are resolved against the
# selector at run time (exact, prefix, or path-coverage successor; see resolve_scope). 2026-09-14 table changes: CT
# and GA whole manuals replace the July slices (run note 2026-09-14-tanf-whole-manuals-md-comar.md); DE DSSM 3000
# added; MD statute and TCA guidance scopes added (selected in the 2026-09-14 cut); ME TANF statute added.
TANF_SCOPES = {
    ...
    "us-ct": [("policy", "2026-09-14-tanf-manual-whole")],
    "us-dc": [("manual", "2026-09-10-tanf-state-policy-manual")],
    "us-de": [("regulation", "2026-07-03-de-tanf-rules"), ("regulation", "2026-09-14-tanf-manual-whole")],
    "us-fl": [("manual", "2026-05-27-fl-ess-manual-r2026-07-15-self-contained")],
    "us-ga": [("manual", "2026-09-14-tanf-manual-whole"), ("guidance", "2026-06-25-ga-tanf")],
    "us-hi": [],
    ...
    "us-md": [("guidance", "2026-07-05-md-tca-2026-guidance"), ("guidance", "2026-07-05-md-tca-guidance"),
              ("statute", "2026-07-03-md-tca-statutes")],
    "us-me": [("regulation", "2026-07-03-me-tanf-rule-125a"), ("statute", "2026-07-03-me-tanf-statute")],
...
# Scopes on disk that the 2026-09-11 union selector did NOT select (the TANF queue marks the state "done" by pointer
# to them; July foundation releases). Resolved against the selector at run time: an entry that is now selected (or
# whose consolidated successor is selected) moves into the searched scopes; only a still-unselected one stays a
# fallback whose hits are EXTRACTABLE.
TANF_UNSELECTED = {
...
# Third field: a selected-scope version suffix that voids the default (the "slice only" notes of GA, CT and DE no
# longer hold once the whole manual is selected; AZ's manual is still behind the Cloudflare wall, run note 2026-09-14).
TANF_DEFAULT_MISSING = {
    "us-ny": ("OUTREACH", "OTDA Temporary Assistance Source Book (TASB.pdf) blocked by an F5/TSPD JavaScript bot wall (queue row, retried 3x incl. US network); only the 2024-2026 TANF State Plan is selected; 18 NYCRR parts 350-397 were not inventoried by the queue", None),
    "us-ga": ("EXTRACTABLE", "Georgia TANF Policy Manual: only sections 1525, 1605, 1615 and Appendix A are in the corpus (47 block rows); the full manual is posted at pamms.dhs.ga.gov/dfcs/tanf/ (queue 'done' by pointer to a 4-section slice)", WHOLE_MANUAL_SUFFIX),
    "us-ct": ("EXTRACTABLE", "CT DSS Uniform Policy Manual: 13 UPM sections plus the 2026 standards chart are in the corpus (28 rows); the full UPM (TFA chapters 1000-8000) is posted by DSS (queue 'done' by pointer to a slice)", WHOLE_MANUAL_SUFFIX),
    "us-az": ("EXTRACTABLE", "AZ DES FAA policy manual: 7 topic pages plus the 143-row FAA5 recovery scope are in the corpus; the manual (dbmefaapolicy.azdes.gov, FAA1-FAA6) is posted whole (queue 'done' by pointer to a slice; re-probed 2026-09-14: Cloudflare challenge to plain and impersonated clients)", None),
    "us-de": ("EXTRACTABLE", "Delaware selected scope us-de/regulation/2026-07-03-de-tanf-rules holds DSSM section 4000 (Financial Responsibility, 41 rows) only; the DSSM 3000 series (TANF program rules: time limits, contract of mutual responsibility, work, sanctions, teen parents, diversion) is posted by the same publisher (regulations.delaware.gov, Title 16) and was not taken (the queue row's 'DSSM 3000 already ingested' names the wrong section)", WHOLE_MANUAL_SUFFIX),
}
# Per-jurisdiction rulebook notes appended to REVIEW cells (from reading the scope shapes 2026-09-12). The MD and ME
# notes hold only while the July rulebook scope is unselected (TANF_JUR_NOTES_UNLESS); the NC note is the 09-11
# unselected-scope note, kept because only section 114 of the Work First Manual was ever taken.
TANF_JUR_NOTES_UNLESS = {"us-md": ("regulation", "2026-07-03-md-tca-comar-publication-2026-06-29-title-07-subtitle-03-chapter-03"),
                         "us-me": ("regulation", "2026-07-03-me-tanf-regulation")}
TANF_JUR_NOTES = {
    "us-nc": "Work First Manual: only section 114 (Income and Budgeting, 18 rows) was ever ingested (us-nc/manual/2026-07-03-nc-work-first-manual); the other WF sections are posted at policies.ncdhhs.gov (queue row: follow-up)",
...
# manuals carry no subsidy chapter). Versions are resolved against the selector like TANF_SCOPES.
CCDF_RULEBOOK_ALLOW = {
...
def body_paths(base: Path, jur: str, dc: str, ver: str) -> set[str]:
    return {r["citation_path"] for r in (load_rows(base, jur, dc, ver) or []) if r.get("body")}


_RESOLVED: dict[tuple, str | None] = {}


def resolve_scope(base: Path, selected: set, jur: str, dc: str, ver: str, warn: bool = True) -> str | None:
    """Selected version standing for a table scope: exact; else the one selected version of the same jurisdiction
    and class that starts with it; else the selected version of that class holding >= 90 percent of the table
    scope's body-bearing citation paths (a consolidation or supersede). None (with a stderr warning) otherwise."""
    key = (jur, dc, ver)
    if key in _RESOLVED:
    ...
    if key in selected:
        ...
        same = sorted(v for j, d, v in selected if j == jur and d == dc)
        pref = [v for v in same if v.startswith(ver)]
        if len(pref) == 1:
            out = pref[0]
        else:
            old = body_paths(base, jur, dc, ver)
            if old:
                cover = [(len(old & body_paths(base, jur, dc, v)) / len(old), v) for v in same]
                cover = [c for c in cover if c[0] >= 0.9]
                if cover:
                    out = max(cover)[1]
    if out is None and warn:
        print(f"warning: {jur}/{dc}/{ver} is not selected and has no selected successor", file=sys.stderr)
    _RESOLVED[key] = out
    return out


def resolve_scopes(base: Path, selected: set, jur: str, scopes):
    """Resolve a list of (dc, ver[, extra...]) table entries; unresolved entries are dropped."""
    out = []
    for entry in scopes:
        dc, ver = entry[0], entry[1]
        r = resolve_scope(base, selected, jur, dc, ver)
        if r is not None:
            out.append((dc, r) + tuple(entry[2:]))
    return out


def selected_by_suffix(selected: set, jur: str, dc: str, suffix: str) -> str | None:
    """Newest selected version of (jur, dc) whose version ends with suffix, or None."""
    c = sorted(v for j, d, v in selected if j == jur and d == dc and v.endswith(suffix))
    return c[-1] if c else None


def first_body_row(rows):
    return next((r for r in rows or [] if (r.get("body") or "").strip()), None)
...
def cfr_part_rows(base: Path, selected: set, part: str):
    """(scope_version, {path segment: row}) for the selected us/regulation scope of 45 CFR <part>, or (None, {})."""
    ver = selected_by_suffix(selected, "us", "regulation", f"title-45-part-{part}")
    if ver is None:
        return None, {}
    rows = load_rows(base, "us", "regulation", ver) or []
    return f"us/regulation/{ver}", {r["citation_path"].split("/")[-1]: r for r in rows if re.match(r"^\d", r["citation_path"].split("/")[-1])}


def usc_section_row(base: Path, selected: set, title: str, sec: str):
    """(scope_version, row) of us/statute/<title>/<sec> with a body, from the newest selected us/statute scope holding it."""
    for ver in sorted((v for j, d, v in selected if j == "us" and d == "statute"), reverse=True):
        for r in load_rows(base, "us", "statute", ver) or []:
            if r["citation_path"] == f"us/statute/{title}/{sec}" and (r.get("body") or "").strip():
                return f"us/statute/{ver}", r
    return None, None


def expand_usc_group(citation: str) -> list[str]:
    """'42 U.S.C. 9858d-9858m' -> ['9858d', ..., '9858m']; '42 U.S.C. 9857-9857a' -> ['9857', '9857a']; '42 U.S.C. 9858' -> ['9858']."""
    spec = citation.split()[-1]
    if "-" not in spec:
        return [spec]
    a, b = spec.split("-")
    ma, mb = re.match(r"(\d+)([a-z]?)", a), re.match(r"(\d+)([a-z]?)", b)
    if not ma.group(2):  # 9857-9857a: the bare section and its lettered sibling
        return [a, b]
...
def state_plan_scan(base: Path, jur: str, selected: set):
    ...
    for p in glob.glob(str(base / "provisions" / jur / "*" / "*.jsonl")):
        ...
        for r in load_rows(base, jur, dc, ver) or []:
            cp = r["citation_path"]
            if "state-plan" not in cp and "/tanf-plan/" not in cp:  # 2026-09-13 ACF family: us-xx/policy/acf/tanf-plan/<period>
                continue
...
# --------------------------------------------------------------------------------------
def build_tanf(base: Path, selector_scopes, out: Path, structure: Path, ctx: dict):
    t0 = time.time()
    cfr = cfr_sections(structure, {"260", "261", "262", "263", "264", "265"})
    # federal layers, looked up in the selected scopes (computed corpus-status strings; element lists unchanged)
    usc = {c.split()[-1]: usc_section_row(base, selector_scopes, "42", c.split()[-1]) for c, _ in TANF_STATUTE}
    usc_scopes = sorted({sv for sv, _ in usc.values() if sv})
    n_usc = sum(1 for sv, _ in usc.values() if sv)
    cfr_rows = {part: cfr_part_rows(base, selector_scopes, part) for part in ("260", "261", "262", "263", "264", "265")}
    cfr_scopes = sorted(sv for sv, _ in cfr_rows.values() if sv)

    def usc_status(sec):
        sv, r = usc[sec]
        return f"present: {sv} row {r['citation_path']} ({len(r['body'])} chars)" if sv else f"absent from every selected us/statute scope (no us/statute/42/{sec} row with a body)"

    def cfr_status(part, sec):
        sv, by = cfr_rows[part]
        r = by.get(sec.split(".")[-1])
        if sv and r and (r.get("body") or "").strip():
            return f"present: {sv} row {r['citation_path']} ({len(r['body'])} chars)"
        return f"not in the corpus: no selected us/regulation scope ends with title-45-part-{part}" if not sv else f"section listed in the eCFR structure but no non-empty body in {sv}"
    ...
    schema = {
        ...
        "method": (
            f"Elements are enumerated from the program's own legal structure: (1) 42 U.S.C. 601-619 section by section ({n_usc} of {len(TANF_STATUTE)} held in {', '.join(usc_scopes) or 'no selected scope'}); "
        ...
        "federal_statute_elements": [
            {"id": f"tanf-f-usc-{c.split()[-1]}", "citation": c, "name": n, "level": "federal" + (" (state option)" if c.split()[-1] in ("602", "607", "608") else ""),
             ...
             "corpus_family_status": usc_status(c.split()[-1]),
        ...
        "federal_regulation_elements": [
            {"id": f"tanf-f-cfr-{sec}", "citation": f"45 CFR {sec}", "part": part, "name": desc,
             ...
             "corpus_family_status": cfr_status(part, sec),
             "pe_modeled": "no"}
        ...
        "schema_uncertainties": [
            ("Section-level elements for 42 U.S.C. 601-619 were enumerated from the compiler's knowledge of the title on 2026-09-11; "
             + (f"the text is now held ({', '.join(usc_scopes)}), which also carries 603a, 604a and 608a outside the element list; the element list is kept as enumerated so the numbers compare"
                if usc_scopes else "the text is not in the corpus; headings should be re-checked against the ingested text once extracted")),
    ...
    for c, n in TANF_STATUTE:
        sec = c.split()[-1]
        pe = "no" if sec not in ("607", "608") else "partial"
        sv, r = usc[sec]
        if sv:
            rows.append(["us", f"tanf-f-usc-{sec}", "federal", "federal statute", "PRESENT", sv, r["citation_path"], pe,
                         f"{c} {n}: heading='{(r.get('heading') or '')[:80]}' | body {len(r['body'])} chars (subsection children below it); inherited by all 51 jurisdictions"])
        else:
            rows.append(["us", f"tanf-f-usc-{sec}", "federal", "federal statute", "EXTRACTABLE", "", "", pe,
                         f"{c} {n}: not in any selected us/statute scope; uscode.house.gov publishes title 42 (the corpus already takes USC titles from it); inherited by all 51 jurisdictions"])
    for part, sec, desc in cfr:
        lvl = "federal" + (" (state option)" if sec in CFR_STATE_OPTION else "")
        sv, by = cfr_rows[part]
        r = by.get(sec.split(".")[-1])  # structure identifier 260.30 -> path segment 30 (same lookup as part 98)
        if sv and r and (r.get("body") or "").strip():
            rows.append(["us", f"tanf-f-cfr-{sec}", lvl, "federal regulation", "PRESENT", sv, r["citation_path"], "no",
                         f"45 CFR {sec} ({desc}): body {len(r['body'])} chars: '{re.sub(chr(10), ' ', r['body'][:90])}'; inherited by all 51 jurisdictions"])
        elif sv:
            rows.append(["us", f"tanf-f-cfr-{sec}", lvl, "federal regulation", "REVIEW", "", "", "no",
                         f"45 CFR {sec} ({desc}): section listed in the eCFR structure but no non-empty body found in {sv}"])
        else:
            rows.append(["us", f"tanf-f-cfr-{sec}", lvl, "federal regulation", "EXTRACTABLE", "", "", "no",
                         f"45 CFR {sec} ({desc}): part {part} not in a selected scope; eCFR Versioner API serves title 45 (extract-ecfr); inherited by all 51 jurisdictions"])
    ...
    for jur in JURS[1:]:
        ...
        scopes = resolve_scopes(base, selector_scopes, jur, TANF_SCOPES.get(jur, []))
        # July scopes the 2026-09-11 selector left out: searched as selected scopes once the selector (or a
        # consolidated successor) holds them; only still-unselected ones stay EXTRACTABLE fallbacks
        unselected = []
        for dc, ver in TANF_UNSELECTED.get(jur, []):
            r = resolve_scope(base, selector_scopes, jur, dc, ver, warn=False)
            if r is not None:
                scopes.append((dc, r))
            else:
                unselected.append((dc, ver))
        jnote = TANF_JUR_NOTES.get(jur)
        if jur in TANF_JUR_NOTES_UNLESS and resolve_scope(base, selector_scopes, jur, *TANF_JUR_NOTES_UNLESS[jur], warn=False):
            jnote = None
        default_missing = TANF_DEFAULT_MISSING.get(jur)
        if default_missing and default_missing[2] and any(v.endswith(default_missing[2]) for _, v in scopes):
        ...
        for e in TANF_STATE_ELEMENTS:
            ...
            fb = search_scopes(base, jur, unselected, e, None)
            if fb and fb[1]:
...
def build_ccdf(base: Path, selector_scopes, out: Path, structure: Path, ctx: dict | None = None):
    t0 = time.time()
    ...
    for jur in JURS[1:]:
        ...
        for e in CCDF_STATE_ELEMENTS:
            ...
            if e["id"] == "ccdf-s26" and jur == "us-ga":
            ...
            elif e["id"] == "ccdf-s27" and selected_by_suffix(selector_scopes, jur, "policy", APPENDIX_SUFFIX):
                av = selected_by_suffix(selector_scopes, jur, "policy", APPENDIX_SUFFIX)
                arows = load_rows(base, jur, "policy", av) or []
                fr = first_body_row(arows)
                status, sv, cp = "PRESENT", f"{jur}/policy/{av}", fr["citation_path"] if fr else arows[0]["citation_path"]
                note = f"ACF-posted Appendix 1 (FY2025-2027) taken 2026-09-13: {len(arows)} rows; body opened"
            elif e["id"] == "ccdf-s27":
            ...
            elif e["id"] == "ccdf-s28" and selected_by_suffix(selector_scopes, jur, "policy", AMENDMENT_SUFFIX):
                mv = selected_by_suffix(selector_scopes, jur, "policy", AMENDMENT_SUFFIX)
                mrows = load_rows(base, jur, "policy", mv) or []
                fr = first_body_row(mrows)
                status, sv, cp = "PRESENT", f"{jur}/policy/{mv}", fr["citation_path"] if fr else mrows[0]["citation_path"]
                note = f"ACF-posted approved plan amendment(s) taken 2026-09-13: {len(mrows)} rows; body opened"
            elif e["id"] == "ccdf-s28":
                ...
                for dc, ver, prefix in CCDF_RULEBOOK_ALLOW.get(jur, []):
                    ver = resolve_scope(base, selector_scopes, jur, dc, ver, warn=False) or ver
                    for r in (load_rows(base, jur, dc, ver) or []):
                        body = r.get("body") or ""
            ...
            elif e["id"] == "ccdf-s30" and selected_by_suffix(selector_scopes, jur, "policy", RATE_SUFFIX):
                rv = selected_by_suffix(selector_scopes, jur, "policy", RATE_SUFFIX)
                rrows = load_rows(base, jur, "policy", rv) or []
                fr = first_body_row(rrows)
                status, sv, cp = "PRESENT", f"{jur}/policy/{rv}", fr["citation_path"] if fr else rrows[0]["citation_path"]
                note = f"current-year rate/copay schedule taken 2026-09-13: {len(rrows)} rows; body opened"
            elif e["id"] == "ccdf-s30":
...
def main():
    ...
    (out / SUMMARY_NAME).write_text(json.dumps(summary, indent=1))
