#!/usr/bin/env python3
"""Needs-driven closure check for SNAP and WIC over the 50 states plus DC (2026-09-11 pass; re-run 2026-09-14
against the sixth cut, manifests/releases/us-rulespec-2026-09-14-wave4-r2-union.json).

Re-run (read-only over data/corpus; writes only into --out):

    python3 docs/coverage/needs-closure-2026-09-14/check_snap_wic.py \
        ...
        --out docs/coverage/needs-closure-2026-09-14
...
def snap_scopes_for(j, eff):
    ...
    for jj, dc, v in eff:
        if jj != j:
            continue
        if j == "us":
            if dc == "statute" and "title-7-consolidated" in v or dc == "regulation" and ("7-cfr-273" in v or "title-7-part-" in v or v == "2026-07-13-recovery-r2026-07-17-dedup") or dc == "guidance" and ("snap" in v or v == "2026-07-13-recovery"):
                out.append((dc, v))
            continue
...
def wic_scopes_for(j, eff):
    ...
    for jj, dc, v in eff:
        if jj != j:
            continue
        if j == "us" and ((dc == "guidance" and "wic" in v) or (dc == "statute" and "wic-statute" in v) or (dc == "regulation" and v == "2026-07-13-recovery-r2026-07-17-dedup")) or j != "us" and ("wic-state-policy-manual" in v or "wic-state-plan" in v):
            out.append((dc, v))
    return sorted(set(out))
...
WIC_STATE_PLAN_HEADING = re.compile(r"WIC.{0,60}State Plan|State Plan of (Program )?Operations", re.I)
...
def main():
    ...
    def federal_gap(program, el):
        ...
        if el["family"] == "federal_regulation":
            held = sorted({r["citation_path"].split("/")[3] for k in scope_rows if k[0] == "us" and k[1] == "regulation" for r in scope_rows[k] if r["citation_path"].startswith("us/regulation/7/")}, key=int)
            return "EXTRACTABLE", "", "", f"part in no selected scope (selected federal scopes hold 7 CFR {', '.join(held)} only); eCFR title 7 chapter II subchapter C lists it (structure API read 2026-09-12); same extractor as us/regulation/2026-06-15-title-7-part-275"
    ...
    def wic_state_rule(j, el, jscopes, nrows):
        ...
        if el["id"] == "wic_s30":
            docs = [r for r in rows_for(jscopes, j) if r["kind"] == "document" and "wic-state-plan" in r["version"] and WIC_STATE_PLAN_HEADING.search(r.get("heading") or "")]
            if docs:
                d = docs[0]
                kids = [r for r in rows_for(jscopes, j) if r["citation_path"].startswith(d["citation_path"]) and (r.get("body") or "").strip()]
                return "PRESENT", d["version"], d["citation_path"], f"document heading {d.get('heading')!r} with {len(kids)} body rows"
    ...
    for program, elements, scopes, rule in (("snap", snap_elements, snap_scopes, snap_state_rule), ("wic", wic_elements, wic_scopes, wic_state_rule)):
        ...
        schema = {
            "program": program, "generated": started, "generator": f"{args.out}/check_snap_wic.py", "selector": str(args.selector),
    ...
    results["selector"] = str(args.selector)
    json.dump(results, open(out / "summary-snap-wic.json", "w"), indent=1)
