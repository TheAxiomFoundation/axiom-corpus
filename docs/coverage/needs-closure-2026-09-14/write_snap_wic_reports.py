#!/usr/bin/env python3
"""Write snap.md and wic.md from the matrices, schemas and summary-snap-wic.json produced by check_snap_wic.py.

    python3 docs/coverage/needs-closure-2026-09-14/write_snap_wic_reports.py \
        --dir docs/coverage/needs-closure-2026-09-14
...
def load(d: Path, program: str):
    ...
    summary = json.load(open(d / "summary-snap-wic.json"))
...
def write(program, d: Path, narrative):
    ...
    lines = []
    lines.append(f"# {name}: needs-driven closure check (2026-09-11 schema, re-run 2026-09-14 on the sixth cut)\n")
