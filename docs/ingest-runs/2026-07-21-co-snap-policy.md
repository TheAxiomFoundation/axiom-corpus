...
The run retained 13 files totaling 6,317,575 bytes and generated 598 rows: 13
document roots plus 585 HTML blocks/PDF pages. Coverage is complete at 598 of
...
Two consecutive live extractions on July 21 produced the same current files,
which are 621 bytes smaller while preserving the exact audited 13-root and
585-child row boundary. Tests ratchet the stable live bytes and per-source hashes.
